...
def signin():
    ...
    if config.CONFIG_DB.has_email_authentication:
        ...
        if form.validate_on_submit():
            ...
            if result is None:
                form.email.errors.append(u'Цахим шуудан эсвэл нууц үг буруу байна')
            if result is False:
...
def signup():
    ...
    if config.CONFIG_DB.has_email_authentication:
        ...
        if form.validate_on_submit():
            ...
            if user_db:
                form.email.errors.append(u'Энэ цахим шуудан бүртгэлтэй байна.')
    ...
    title = u'Бүртгүүлэх' if config.CONFIG_DB.has_email_authentication else u'Нэвтрэх'
    return flask.render_template(
        'auth/auth.html',
        title=title,
        html_class='auth',
...
def signout():
    login.logout_user()
    flask.flash(u'Амжилттай гарлаа.', category='success')
...
def get_user_db_from_email(email, password):
    ...
    if not user_dbs:
        return None
    if len(user_dbs) > 1:
        flask.flash(u'''We are sorry but it looks like there is a conflict with
