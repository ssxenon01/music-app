...
from api.pyItunes.Song import Song
from api.pyItunes.Playlist import Playlist, PlTrack
        ...
        for trackid, attributes in self.il['Tracks'].iteritems():
            ...
            if attributes.get('Location'):
                if ( self.musicPathXML is None or self.musicPathSystem is None ):
                    s.location = unicode(urlparse.unquote(urlparse.urlparse(attributes.get('Location')).path[1:]),
                                         "utf8")
                else:
                    s.location = unicode(
                        urlparse.unquote(urlparse.urlparse(attributes.get('Location')).path[1:]).replace(
                            self.musicPathXML, self.musicPathSystem), "utf8")
